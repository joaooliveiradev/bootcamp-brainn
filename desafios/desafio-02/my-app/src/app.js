...
const App = () => {
    const [postContent, setPostContent] = useState(posts.map(post => ({ id: post.id, title: post.title, content: post.content, displayContent: false })));
    const [displayState, setdisplayState] = useState({ id: 0 });
    function showPost(clickedId) {
        const updatePostObj = postContent.filter(post => post.id === clickedId).map(postFiltered => ({ ...postFiltered, displayContent: !postFiltered.displayContent }));
        const falsePostsObj = postContent.filter(post => post.id !== clickedId);
        const updatePostArr = updatePostObj.concat(falsePostsObj).sort((a, b) => a.id - b.id);
        return { postUpdated: updatePostArr, postUpdateId: updatePostObj[0].id };
    }
    function updateContentState(clickId) {
        if (displayState.id === 0) {
            const postUpdate = showPost(clickId);
            updateStatePost(postUpdate.postUpdateId);
            return postUpdate.postUpdated;
        } else {
            if (clickId !== displayState.id) {
                return postContent;
            } else {
                updateStatePost(0)
                const postUpdate = showPost(clickId);
    ...
    function updateStatePost(clickId) {
        return setdisplayState(({ id: clickId }))
    }
    function handleClick(event, clickId) {
        event.preventDefault();
        setPostContent(updateContentState(clickId));
    }
    return (
        <>
            <Header children="TudoMassa" />
            <Nav children="Esse exercício foi muito Massa de fazer" />
            <Aside>
                <LinkList state={{ postContent, setPostContent }} handleClick={handleClick} className="LinksList" />
            </Aside>
            <Main>
                <Section>
                    <Post state={{ postContent, displayState }} />
                </Section>
            </Main>
